...
A client's way to provide proof to a server that it's authenticated with them. The server issues a token in exchange for the correct login credentials.
- [x] What steps can you take in your web apps to keep your data secure?
Setting up Private Routes, Using Client-Side Authentication to gain access
- [x] Describe how web servers work.
A server is connected to the internet and stores code for different websites to be shared across the web at all times. A server is what hosts our websites/apps & serves the web pages upon request (i.e. user typing in domain name).
